...
namespace PointFormat
{
    ...
    public class cPointData
    {
        ...
        public double GetMinX()
        {
            ...
            {
                value = Math.Min(value, PointData[i].X);
            }
        ...
        public double GetMinY()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                value = Math.Min(value, PointData[i].Y);
            }
        ...
        public double GetMinZ()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                value = Math.Min(value, PointData[i].Z);
            }
        ...
        public double GetMaxX()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                value = Math.Max(value, PointData[i].X);
            }
        ...
        public double GetMaxY()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                value = Math.Max(value, PointData[i].Y);
            }
        ...
        public double GetMaxZ()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                value = Math.Max(value, PointData[i].Z);
            }
        ...
        #region ToSimple
        /// <summary>
        /// X,Y,Z座標の長さがsize以下になるようにリサイズし、点の座標を四捨五入して重複した点を削除してシンプルにします。
        /// </summary>
        /// <param name="size">指定サイズ</param>
        public void ToSimple(int size)
        {
            List<cPoint> simplePointList = new List<cPoint>();
            int i, j;
            double x, y, z;
            bool flag;

            #region X,Y,Zのうち最も長い幅を取得し、縮小倍率を求める。
            int xLength = (int)(GetMaxX() - GetMinX());
            int yLength = (int)(GetMaxY() - GetMinY());
            int zLength = (int)(GetMaxZ() - GetMinZ());
            double maxLength = Math.Max(Math.Max(xLength, yLength), zLength);
            //縮小倍率
            double magnification = size / maxLength;
            #endregion

            for (i = 0; i < PointData.Length; i++)
            {
                flag = false;
                x = Math.Round(PointData[i].X * magnification);
                y = Math.Round(PointData[i].Y * magnification);
                z = Math.Round(PointData[i].Z * magnification);

                for (j = 0; j < simplePointList.Count; j++)
                {
                    if (x == simplePointList[j].X && y == simplePointList[j].Y && z == simplePointList[j].Z)
                    {
                        flag = true;
                        break;
                    }
                ...
                if (flag == false)
                {
                    simplePointList.Add(new cPoint(x, y, z, PointData[i].R, PointData[i].G, PointData[i].B));
                }
            }

            PointData = simplePointList.ToArray();
        }
        #endregion
