...
namespace cBitmap
{
    public class cCreateBitmapFrom3DPointFast:cCreateBitmapFrom3DPoint
    {
        ...
        public cCreateBitmapFrom3DPointFast(cPointData PointData, int PixelSize, double Ratio)
            : base(PointData, Ratio)
        {
            base.DrawMethod = this.DrawMethodFast;
            this.PixelSize = PixelSize;
        }
        ...
        protected virtual Bitmap DrawMethodFast(Color[,] ColorArray)
        {
            ...
            double[,] drewPointZ = new double[iXMax, iYMax];
            for (int i = 0; i < iXMax; i++)
            {
                for (int j = 0; j < iYMax; j++)
                {
                    drewPointZ[i, j] = double.MinValue;
                }
            }
            unsafe
            {
                ...
                for (int i = Offset; i < iYMax - Offset; i++)
                    for (int j = Offset; j < iXMax - Offset; j++)
                    {
                        ...
                        for (int SmallY = -(Offset); SmallY < (Offset); SmallY++)
                            ...
                            {
                                if (daZPoint[j, i] > drewPointZ[j + SmallX, i + SmallY])
                                {
                                    drewPointZ[j + SmallX, i + SmallY] = daZPoint[j, i];
